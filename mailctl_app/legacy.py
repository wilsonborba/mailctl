...
def command_install(args: argparse.Namespace) -> int:
    ...
    host = "127.0.0.1" if getattr(args, "localhost_only", False) else getattr(args, "host", runtime_service.DEFAULT_API_HOST)
    port = getattr(args, "port", runtime_service.DEFAULT_API_PORT)
    profile = getattr(args, "profile", "cli")
    runtime_service.save_service_config(host=host, port=port, profile=profile)
    manifest = runtime_service.save_install_manifest(profile)
    service_info = None
    notes: list[str] = []
    if profile in {"cli+api", "full"}:
        service_info = runtime_service.ensure_systemd_ready(host=host, port=port)
        notes.append("API systemd user unit installed; activate it with `mailctl api start`.")
    if profile == "full":
        notes.append("`full` currently installs the CLI and API foundations only; front-end remains reserved for a future phase.")
    ...
    result = {
        ...
        "api_service": service_info,
        "api_defaults": {"host": host, "port": port},
...
def command_uninstall(args: argparse.Namespace) -> int:
    ...
    unit_path = runtime_service.systemd_unit_path()
    if not target.exists() and not runtime.exists() and not unit_path.exists():
        raise MailCtlError(
    ...
    if target.exists():
        target.unlink()
    removed = [str(target), str(runtime)]
    if runtime_service.has_systemctl():
        runtime_service.run_systemctl("disable", "--now", runtime_service.SERVICE_NAME)
        runtime_service.run_systemctl("daemon-reload")
    if unit_path.exists():
        unit_path.unlink()
        removed.append(str(unit_path))
    if runtime.exists():
        shutil.rmtree(runtime)
    return output_result(args, "uninstall", {"removed": removed})




def resolve_api_host(args: argparse.Namespace) -> str:
    return "127.0.0.1" if getattr(args, "localhost_only", False) else (args.host or runtime_service.load_service_config()["host"])


def resolve_api_port(args: argparse.Namespace) -> int:
    requested_port = args.port or runtime_service.load_service_config()["port"]
    host = resolve_api_host(args)
    selected_port, fallback_used = runtime_service.find_available_port(host, requested_port)
    runtime_service.save_service_config(host=host, port=selected_port)
    setattr(args, "_resolved_api_port", selected_port)
    setattr(args, "_requested_api_port", requested_port)
    setattr(args, "_api_port_fallback_used", fallback_used)
...
def command_api_serve(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        raise MailCtlError(
            f"API dependencies are missing: {exc}",
            exit_code=EXIT_CONFIG,
            error_type="configuration_error",
        ) from exc
    if getattr(args, "_api_port_fallback_used", False):
        print(json_dump({"ok": True, "command": "api_serve", "result": {"requested_port": getattr(args, "_requested_api_port", port), "selected_port": port, "fallback_used": True, "host": host}}))
...
def command_api_start(args: argparse.Namespace) -> int:
    host = resolve_api_host(args)
    port = args.port or runtime_service.load_service_config()["port"]
    result = runtime_service.service_start(host=host, port=port)
    if result["start_returncode"] != 0:
        raise MailCtlError(
            result["start_stderr"] or "failed to start API service",
            exit_code=EXIT_SERVER,
            error_type="server_error",
        )
    return output_result(args, "api_start", result)


def command_api_stop(args: argparse.Namespace) -> int:
    result = runtime_service.service_stop()
    if result["stop_returncode"] not in {0, 5}:
        raise MailCtlError(
            result["stop_stderr"] or "failed to stop API service",
            exit_code=EXIT_SERVER,
            error_type="server_error",
        )
    return output_result(args, "api_stop", result)


def command_api_restart(args: argparse.Namespace) -> int:
    host = resolve_api_host(args)
    port = args.port or runtime_service.load_service_config()["port"]
    result = runtime_service.service_restart(host=host, port=port)
    if result["restart_returncode"] != 0:
        raise MailCtlError(
            result["restart_stderr"] or "failed to restart API service",
            exit_code=EXIT_SERVER,
            error_type="server_error",
        )
    return output_result(args, "api_restart", result)


def command_api_status(args: argparse.Namespace) -> int:
    return output_result(args, "api_status", runtime_service.service_status())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    install_cmd.add_argument("--profile", choices=["cli", "cli+api", "full"], default="cli")
    install_cmd.add_argument("--host", default=runtime_service.DEFAULT_API_HOST)
    install_cmd.add_argument("--port", type=int, default=runtime_service.DEFAULT_API_PORT)
    install_cmd.add_argument("--localhost-only", action="store_true")
    ...
    api_cmd = subparsers.add_parser("api")
    api_sub = api_cmd.add_subparsers(dest="api_command", required=True)
    api_serve = api_sub.add_parser("serve")
    api_serve.add_argument("--host")
    api_serve.add_argument("--port", type=int)
    api_serve.add_argument("--localhost-only", action="store_true")
    api_serve.set_defaults(func=command_api_serve)
    api_start = api_sub.add_parser("start")
    add_json_flag(api_start)
    api_start.add_argument("--host")
    api_start.add_argument("--port", type=int)
    api_start.add_argument("--localhost-only", action="store_true")
    api_start.set_defaults(func=command_api_start)
    api_stop = api_sub.add_parser("stop")
    add_json_flag(api_stop)
    api_stop.set_defaults(func=command_api_stop)
    api_restart = api_sub.add_parser("restart")
    add_json_flag(api_restart)
    api_restart.add_argument("--host")
    api_restart.add_argument("--port", type=int)
    api_restart.add_argument("--localhost-only", action="store_true")
    api_restart.set_defaults(func=command_api_restart)
    api_status = api_sub.add_parser("status")
    add_json_flag(api_status)
    api_status.set_defaults(func=command_api_status)
